import { Request, Response } from "express";

// Service
import {
  addBranchPaymentMethodService,
  getBranchPaymentMethodsService,
  getActiveBranchPaymentMethodsService,
  getBranchPaymentMethodByIdService,
  updateBranchPaymentMethodService,
  deleteBranchPaymentMethodService,
  toggleBranchPaymentMethodStatusService,
} from "../service/BranchPaymentMethodService/branchPaymentMethodService";

// Error
import { handleError } from "../helper/responseHelper";

/**
 * POST /branch-payment-methods
 * Menambahkan payment method untuk branch
 */
export const addBranchPaymentMethod = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const {
      branchId,
      method,
      provider,
      isActive,
      accountNumber,
      accountName,
      instructions,
    } = req.body;

    const qrCodeImage = req.file 
      ? `uploads/payment-qrcodes/${req.file.filename}`
      : undefined;

    const newPaymentMethod = await addBranchPaymentMethodService({
      branchId: BigInt(branchId),
      method,
      provider,
      isActive,
      accountNumber,
      accountName,
      qrCodeImage,
      instructions,
    });

    res.status(201).json({
      success: true,
      data: newPaymentMethod,
      message: "Payment method berhasil ditambahkan",
    });
  } catch (error) {
    handleError(error, res);
  }
};

/**
 * GET /branch-payment-methods/:branchId
 * Mendapatkan semua payment methods untuk branch
 */
export const getBranchPaymentMethods = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const { branchId } = req.params;

    const paymentMethods = await getBranchPaymentMethodsService(
      BigInt(branchId),
    );

    res.status(200).json({
      success: true,
      data: paymentMethods,
    });
  } catch (error) {
    handleError(error, res);
  }
};

/**
 * GET /branch-payment-methods/:branchId/active
 * Mendapatkan payment methods yang aktif untuk branch
 */
export const getActiveBranchPaymentMethods = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const { branchId } = req.params;

    const paymentMethods = await getActiveBranchPaymentMethodsService(
      BigInt(branchId),
    );

    res.status(200).json({
      success: true,
      data: paymentMethods,
    });
  } catch (error) {
    handleError(error, res);
  }
};

/**
 * GET /branch-payment-methods/detail/:id
 * Mendapatkan detail payment method berdasarkan ID
 */
export const getBranchPaymentMethodById = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const { id } = req.params;

    const paymentMethod = await getBranchPaymentMethodByIdService(BigInt(id));

    res.status(200).json({
      success: true,
      data: paymentMethod,
    });
  } catch (error) {
    handleError(error, res);
  }
};

/**
 * PUT /branch-payment-methods/:id
 * Memperbarui payment method
 */
export const updateBranchPaymentMethod = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const { id } = req.params;
    const {
      method,
      provider,
      isActive,
      accountNumber,
      accountName,
      instructions,
      displayOrder,
    } = req.body;

    const qrCodeImage = req.file 
      ? `uploads/payment-qrcodes/${req.file.filename}`
      : undefined;

    const updatedPaymentMethod = await updateBranchPaymentMethodService({
      id: BigInt(id),
      method,
      provider,
      isActive,
      accountNumber,
      accountName,
      qrCodeImage,
      instructions,
      displayOrder,
    });

    res.status(200).json({
      success: true,
      data: updatedPaymentMethod,
      message: "Payment method berhasil diperbarui",
    });
  } catch (error) {
    handleError(error, res);
  }
};

/**
 * DELETE /branch-payment-methods/:id
 * Menghapus payment method
 */
export const deleteBranchPaymentMethod = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const { id } = req.params;

    await deleteBranchPaymentMethodService(BigInt(id));

    res.status(200).json({
      success: true,
      message: "Payment method berhasil dihapus",
    });
  } catch (error) {
    handleError(error, res);
  }
};

/**
 * PATCH /branch-payment-methods/:id/toggle-status
 * Toggle payment method active status
 */
export const toggleBranchPaymentMethodStatus = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const { id } = req.params;

    const updatedPaymentMethod = await toggleBranchPaymentMethodStatusService(
      BigInt(id),
    );

    res.status(200).json({
      success: true,
      data: updatedPaymentMethod,
      message: `Payment method berhasil ${updatedPaymentMethod.isActive ? "diaktifkan" : "dinonaktifkan"}`,
    });
  } catch (error) {
    handleError(error, res);
  }
};

