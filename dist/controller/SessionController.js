...
exports.updateSession = exports.getSessionById = exports.getSessions = exports.createSession = void 0;
...
const createSession = async (req, res) => {
    try {
        ...
        const serializedSession = JSON.parse(JSON.stringify(session, (_key, value) => typeof value === "bigint" ? value.toString() : value));
        res.status(201).json({
            success: true,
...
const getSessions = async (req, res) => {
    try {
        ...
        if (userRole === "admin") {
            ...
            sessions = await prisma_1.default.session.findMany({
                where: {
                    order: {
                        branchId: admin.branchId,
                    },
                    ...(status && { status: status }),
                    ...(deviceId && { roomAndDeviceId: BigInt(deviceId) }),
                },
                include: {
                    order: {
                        include: {
        ...
        const serializedSessions = JSON.parse(JSON.stringify(sessions, (_key, value) => typeof value === "bigint" ? value.toString() : value));
        res.status(200).json({
            success: true,
...
const getSessionById = async (req, res) => {
    try {
        ...
        if (userRole === "customer") {
            ...
            const hasAccess = await (0, checkBranchAccessHelper_1.checkBranchAccess)(userId, session.order.branchId);
            if (!hasAccess) {
                res.status(403).json({
                    success: false,
                    message: "Anda tidak memiliki akses ke session ini",
...
const updateSession = async (req, res) => {
    try {
        ...
        const serializedSession = JSON.parse(JSON.stringify(updatedSession, (_key, value) => typeof value === "bigint" ? value.toString() : value));
        res.status(200).json({
            success: true,
