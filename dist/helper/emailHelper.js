...
const sendOTPEmail = async ({ to, otp, expiresInMinutes, purpose = "login", }) => {
    try {
        ...
        if (OTP_DELIVERY_METHOD === "console") {
            ...
            console.log(`⏱️  Expires at: ${new Date(Date.now() + expiresInMinutes * 60 * 1000).toLocaleString()}`);
            console.log("=".repeat(60) + "\n");
            return true;
        }
        ...
        const titleText = purpose === "reset password" ? "Reset Your Password" : "Your OTP Code";
        const bodyText = purpose === "reset password"
