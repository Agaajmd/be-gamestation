...
exports.syncNationalHolidaysService = syncNationalHolidaysService;
exports.addCustomHolidayService = addCustomHolidayService;
exports.addCustomHolidaysService = addCustomHolidaysService;
exports.getBranchHolidaysService = getBranchHolidaysService;
...
async function syncNationalHolidaysService(payload) {
    ...
    if (branchIds && branchIds.length > 0) {
        const bigIntIds = branchIds.map((id) => BigInt(id));
        branches = await database_1.prisma.branch.findMany({
    ...
    if (!response.ok) {
        throw new holidayError_1.FailedToFetchHolidaysError();
    }
    const apiData = await response.json();
    // Validate response format
    if (!Array.isArray(apiData)) {
        throw new holidayError_1.FormatResponseInvalidError();
    }
    // Filter hanya libur nasional
    const nationalHolidays = apiData.filter((holiday) => holiday.is_national_holiday === true);
    if (nationalHolidays.length === 0) {
        throw new holidayError_1.NationalHolidayNotFoundError();
    }
    ...
    if (deleteExisting) {
        for (const holiday of nationalHolidays) {
            ...
            const result = await database_1.prisma.branchHoliday.deleteMany({
                where: {
    ...
    for (const holiday of nationalHolidays) {
        try {
            ...
            const existing = await branchHolidayRepository_1.BranchHolidayRepository.findMany({
                branchId: { in: branches.map((b) => b.id) },
                date: holidayDate,
                name: holidayName,
            });
            ...
            if (overwrite && existing.length > 0) {
                await branchHolidayRepository_1.BranchHolidayRepository.updateMany({
            ...
            if (newBranchIds.length > 0) {
                await branchHolidayRepository_1.BranchHolidayRepository.createMany(newBranchIds.map((branchId) => ({
                    branchId,
                    date: holidayDate,
                    name: holidayName,
                    description: description,
...
async function addCustomHolidayService(payload) {
    ...
    for (const branch of branches) {
        try {
            ...
            const existing = await database_1.prisma.branchHoliday.findFirst({
                where: {
                    branchId: branch.id,
                    date: holidayDate,
                    name: name,
            ...
            if (existing) {
                if (overwrite) {
                    ...
                    await database_1.prisma.branchHoliday.update({
                ...
                // Create new holiday
                const holiday = await database_1.prisma.branchHoliday.create({
                    data: {
                        branchId: branch.id,
                        date: holidayDate,
                        name: name,
...
async function addCustomHolidaysService(payload) {
    ...
    for (const holiday of holidays) {
        ...
        if (result.createdList)
            results.createdList.push(...result.createdList);
        if (result.updatedList)
            results.updatedList.push(...result.updatedList);
        if (result.skippedList)
            results.skippedList.push(...result.skippedList);
        if (result.errorList)
            results.errorList.push(...result.errorList);
    }
    return {
...
async function getBranchHolidaysService(payload) {
    ...
    const holidays = await database_1.prisma.branchHoliday.findMany({
        where,
        orderBy: { date: "asc" },
    });
    return holidays;
...
async function deleteBranchHolidayService(payload) {
    const { branchIds, holidayId, startDate, endDate } = payload;
    if (!holidayId && !startDate && !endDate) {
        throw new Error("Minimal harus provide holidayId atau startDate & endDate untuk delete");
    }
    // Get target branches
    let branches;
    if (branchIds && branchIds.length > 0) {
    ...
    if (holidayId) {
        ...
        const holiday = await database_1.prisma.branchHoliday.findUnique({
            where: { id: holidayId },
        });
        if (!holiday) {
        ...
        result = await database_1.prisma.branchHoliday.deleteMany({
            where: {
                id: holidayId,
            },
        });
        ...
        result = await database_1.prisma.branchHoliday.deleteMany({ where });
        return {
            deletedCount: result.count,
